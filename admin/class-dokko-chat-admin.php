...
class Dokko_Chat_Admin {
    ...
    public function shortcode_section_callback() {
        echo '<p>' . __('Copy and paste the shortcode below into your page or post editor to display the embedded chat.', 'dokko-chat') . '</p>';
    }
    ...
    public function dokko_app_shortcode_callback() {
        ...
        // Use a class to control visibility with JavaScript
        $visibility_class = $is_embedded ? '' : 'dokko-hidden-field';
        
        // Create a wrapper to be targeted by JavaScript
        echo '<div class="dokko-shortcode-wrapper ' . $visibility_class . '">';
        
        if (!$is_embedded) {
            echo '<p style="color: #666;">' . __('Shortcode is only available in Embedded mode. Please select "Embedded" above.', 'dokko-chat') . '</p>';
        } else {
            ?>
            <div class="dokko-shortcode-display">
                ...
                <div style="background: #f5f5f5; padding: 10px; border-left: 4px solid #2271b1; margin: 10px 0; border-radius: 4px;">
                    ...
                    <button type="button" class="button" id="dokko-copy-shortcode" style="margin-left: 10px;">Copy</button>
                </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    const copyBtn = document.getElementById('dokko-copy-shortcode');
                    if (copyBtn) {
                        copyBtn.addEventListener('click', function(e) {
                            e.preventDefault();
                            const codeElement = document.getElementById('dokko-shortcode-code');
                            const text = codeElement.textContent;
                            navigator.clipboard.writeText(text).then(function() {
                                const originalText = copyBtn.textContent;
                                copyBtn.textContent = '<?php _e('Copied!', 'dokko-chat'); ?>';
                                setTimeout(function() {
